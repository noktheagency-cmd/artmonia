    ...
    <div className="admin-shell">
      ...
      <div className="admin-main">
        <header className="admin-topbar">
          ...
          <div className="admin-profile"><CircleUserRound /><span><strong>{adminName}</strong><small>Administrator</small></span></div>
        </header>
        ...
        <main className="admin-content">
          ...
          {view === "homepage" ? (
            <section className="admin-view-section">
              <div className="admin-page-title"><div><h1>Ana səhifə</h1><p>Dəyişmək istədiyiniz hissənin adına klikləyin.</p></div></div>
          ...
          {view === "media" ? (
            <section className="admin-view-section">
              ...
              {!media.length ? <div className="admin-empty"><ImageIcon /><h3>Media kitabxanası boşdur</h3><p>İlk şəkli yuxarıdakı sahədən yükləyə bilərsiniz.</p></div> : <div className="media-grid">{media.map((asset) => <article key={asset.id}>{asset.mime_type?.startsWith("video") ? <video src={asset.public_url} muted /> : <img src={asset.public_url} alt={asset.alt_text || asset.name} />}<div><strong>{asset.name}</strong><small>{asset.size_bytes ? `${(asset.size_bytes / 1024 / 1024).toFixed(1)} MB` : "—"}</small></div><button className="media-delete" type="button" onClick={() => removeMedia(asset)} title="Faylı sil"><Trash2 /></button></article>)}</div>}
            </section>
          ) : null}
      ...
      {editing ? (
        <div className="editor-overlay" role="dialog" aria-modal="true" aria-label={`${editing.label} redaktəsi`}>
          <div className="editor-drawer">
            <header><div><span>Ana səhifə redaktoru</span><h2>{editing.label}</h2></div><button type="button" onClick={() => setEditing(null)} aria-label="Bağla"><X /></button></header>
