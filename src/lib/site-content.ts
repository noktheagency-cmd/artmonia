...
import { blankSection, publicTestimonials } from "./section-editing";
...
export async function getPublishedContent(): Promise<SiteContentMap> {
  ...
  if (error) throw new Error("Sayt məlumatları müvəqqəti yüklənmir.");
  ...
  (data ?? []).forEach((row) => {
    ...
    else publishedContent[row.key] = blankSection(defaultContent[row.key]);
  });
...
export async function getAdminSections(): Promise<SiteSectionRecord[]> {
  ...
  if (error) throw new Error("Admin məlumatları yüklənmədi. Səhifəni yeniləyin.");
  ...
  const storedSections = (data as SiteSectionRecord[])
    ...
    .map((section) => {
      ...
      const sanitizedSection = { ...section, label: section.label || definition?.label || section.key, description: section.description || definition?.description || "", content: sanitizeSectionContent(section.key, section.content, data) };
